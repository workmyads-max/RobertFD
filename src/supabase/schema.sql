...
CREATE POLICY "Users can view own ticket messages" ON public.support_messages FOR SELECT USING (EXISTS (SELECT 1 FROM public.support_tickets WHERE public.support_tickets.id = public.support_messages.ticket_id AND public.support_tickets.user_email = (auth.jwt() ->> 'email')::TEXT));
CREATE POLICY "Users can create ticket messages" ON public.support_messages FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM public.support_tickets WHERE public.support_tickets.id = public.support_messages.ticket_id AND public.support_tickets.user_email = (auth.jwt() ->> 'email')::TEXT));
