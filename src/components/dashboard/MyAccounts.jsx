        ...
        <div className="space-y-4 mt-8">
          ...
          {pendingOrders.map((o, i) => (
            <motion.div key={o.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              className="rounded-2xl overflow-hidden"
              style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,92,0,0.25)' }}>
              {/* Top accent bar */}
              <div className="h-0.5" style={{ background: 'linear-gradient(90deg,#FF5C00,#FF7A2F)', opacity: 0.8 }} />
              <div className="p-5">
                {/* Header row */}
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2.5 flex-wrap">
                        <span className="text-sm font-black text-foreground font-mono">{o.order_id || `Order #${o.id?.slice(0,8)}`}</span>
                        <span className="flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold"
                          style={{ background: 'rgba(255,92,0,0.12)', border: '1px solid rgba(255,92,0,0.3)', color: '#FF5C00' }}>
                  ...
                  <div className="text-right flex-shrink-0">
                    <div className="text-lg font-black text-foreground">${(o.price || o.account_size || 0).toLocaleString()}</div>
                    <div className="text-[10px] font-mono text-muted-foreground">Order Total</div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl"
                  ...
                  <span className="text-[11px] text-muted-foreground">
                    Your payment is being reviewed. Account credentials will be delivered within <span className="text-foreground font-semibold">1–24 hours</span> after confirmation.
                  </span>
                </div>
              </div>
            </motion.div>
