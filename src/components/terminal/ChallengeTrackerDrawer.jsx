...
import { Target, Shield, TrendingUp, Calendar, Zap, Clock, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function ChallengeTrackerDrawer({ account, rules, balance, equity, dailyOpenBalance }) {
  ...
  const metrics = [
    { label: 'Profit Target', value: profitPct, max: profitTarget,  color: '#FF5C00',  warn: false,                              icon: Target    },
    { label: 'Daily DD Used', value: dailyDD,   max: dailyDDLimit,  color: '#10b981',  warn: dailyDD >= dailyDDLimit * 0.8,      icon: Shield    },
    { label: 'Max DD Used',   value: maxDD,     max: maxDDLimit,    color: '#8b5cf6',  warn: maxDD   >= maxDDLimit   * 0.8,      icon: TrendingUp },
  ];

  const rules_list = [
    { label: 'News Trading',      val: rules?.newsTrading      },
    { label: 'Overnight Holding', val: rules?.overnightHolding },
    { label: 'Weekend Holding',   val: rules?.weekendHolding   },
    { label: 'EA Allowed',        val: true                    },
  ];

  return (
    <div className="border-t flex-shrink-0" style={{ background: '#080c16', borderColor: 'rgba(255,255,255,0.07)' }}>

      {/* ── Always-visible header strip ─────────────────────────────────────── */}
      <button
        onClick={() => setCollapsed(c => !c)}
        className="w-full flex items-center gap-3 px-4 py-2 hover:bg-white/[0.02] transition-all group"
      ...
      >
        ...
        <div className="hidden lg:flex items-center gap-4">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-black text-blue-400">{tradingDays}/{minDays}d</span>
          </div>
        </div>
        ...
        {/* Toggle icon */}
        <motion.div animate={{ rotate: collapsed ? 0 : 180 }} transition={{ duration: 0.2 }} className="flex-shrink-0">
      ...
      {!collapsed && (
        <div className="px-4 pb-4 pt-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="space-y-3">
            <div className="text-[9px] font-mono uppercase tracking-widest text-slate-500 mb-2 flex items-center gap-1.5">
          ...
          <div>
            <div className="text-[9px] font-mono uppercase tracking-widest text-slate-500 mb-2 flex items-center gap-1.5">
              <Calendar className="w-3 h-3 text-blue-400" /> Trading Days
            </div>
            <div className="grid grid-cols-4 gap-1.5">
              {Array.from({ length: Math.max(minDays, 1) }).map((_, i) => {
          ...
          <div>
            <div className="text-[9px] font-mono uppercase tracking-widest text-slate-500 mb-2 flex items-center gap-1.5">
              <Shield className="w-3 h-3 text-purple-400" /> Account Rules
            </div>
          ...
          <div>
            <div className="text-[9px] font-mono uppercase tracking-widest text-slate-500 mb-2 flex items-center gap-1.5">
              <Clock className="w-3 h-3 text-primary" /> Daily DD Reset
            </div>
