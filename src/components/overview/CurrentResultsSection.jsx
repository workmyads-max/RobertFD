...
import { Target, DollarSign, Info } from 'lucide-react';
...
function buildChartData(trades, accountSize) {
  const closedTrades = trades
    .filter(t => t.status === 'closed' && t.close_time && t.pnl != null)
...
export default function CurrentResultsSection({ account, trades, livePositions }) {
  const [showObjectives, setShowObjectives] = useState(true);
  const [pnlMode, setPnlMode] = useState('absolute');
  ...
  const profitTarget = account?.phase === 'phase2'
    ? (rules.phase2_target || 5)
    : (rules.phase1_target || 10);
  const targetAmount = accountSize * (profitTarget / 100);

  const metrics = [
    { label: 'Balance', value: `$${fmt(balance)}`, icon: DollarSign, color: '#60a5fa' },
    { label: 'Equity', value: `$${fmt(liveEquity)}`, icon: Target, color: liveEquity >= balance ? '#10b981' : '#ef4444' },
    { label: 'Unrealized PnL', value: `${liveUnrealizedPnl >= 0 ? '+' : ''}$${fmt(liveUnrealizedPnl)}`, color: liveUnrealizedPnl >= 0 ? '#10b981' : '#ef4444' }
  ];
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-[10px] font-semibold transition-all ${
              showObjectives ? 'bg-primary/20 text-primary' : 'bg-white/5 text-white/40 hover:text-white/60'
            }`}
          >
          ...
          <select
            ...
            className="px-3 py-1.5 rounded-lg text-[10px] font-semibold bg-white/5 text-white/70 border border-white/10 focus:outline-none focus:border-primary/50"
          >
          ...
          {metrics.map((m) => {
            ...
            return (
              <div key={m.label} className="rounded-xl p-3"
                ...
                <div className="text-lg font-black font-mono" style={{ color: m.color }}>
                  {m.value}
                </div>
              </div>
            );
                ...
                content={({ active, payload }) => {
                  ...
                  return (
                    <div className="px-3 py-2 rounded-xl text-[10px] font-mono shadow-xl"
                      style={{ background: 'rgba(6,10,22,0.98)', border: '1px solid rgba(255,255,255,0.08)' }}>
