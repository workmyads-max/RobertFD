...
Deno.serve(async (req) => {
  try {
    ...
    if (action === 'approve_payment') {
      ...
      // ── B2G1 PROMO: provision extra accounts ─────────────────────────────────
      ...
      //   2. Free promotional account (smaller size)
      if (order.promo_applied && order.promo_type === 'buy2get1' && !order.promo_free_account_provisioned) {
        ...
        // Provision free promotional account
        if (order.promo_free_account_size > 0) {
          try {
            ...
            // Mark free account as provisioned on the order
      ...
      try {
        ...
        console.error('[ManualCrypto] Affiliate commission failed:', e.message);
      }
